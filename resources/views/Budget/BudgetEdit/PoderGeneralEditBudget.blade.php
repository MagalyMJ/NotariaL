...
// Cuando se trata de una persona Fisica, no lleva registro 
	var Hf= document.getElementById("honorariosFisica");
   	if( Hf.checked ){
   		document.getElementById("labelGastos_registro").innerHTML = 'Gestoria de Escritura: 0 por ser Persona Fisica'
   ...
   function PMoral(){ 
	...
	var Hf= document.getElementById("honorariosMoral");
   	if( Hf.checked ){
   		document.getElementById("labelGastos_registro").innerHTML = 'Gestoria de Escritura: $' + cost
	...
	<!--  El costo de honorarios es un valor fijo para este servicio Pero entre 2 opcciones diferentes -->
	 <label for="">Honorarios Por Persona Moral: ${{$Budget->case_service->service->findExpeseCostByName('Honorarios Por Persona Moral')}} </label> 
	 <input name="honorarios" onclick="PMoral()" class="input long" id="honorariosMoral" type="radio" value="{{$Budget->case_service->service->findExpeseCostByName('Honorarios Por Persona Moral')}}" 
	 ...
	 <input name="honorarios" onclick="Pfisica()" class="input long" id="honorariosFisica" type="radio" value="{{$Budget->case_service->service->findExpeseCostByName('Honorarios Por Persona Fisica')}}" 
	...
	<input name="gastos_registro" class="input long" id="gastos_registro" type="hidden" value="" />
